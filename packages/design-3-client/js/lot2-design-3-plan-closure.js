...
const ROOMS={
 A:{
  ...
  upper:[
   ...
   {name:'Potential over-garage option',kind:'option',x:108,y:8,w:20,d:20,optional:true,authorized:false}
  ]
 },
 B:{
  ground:[
...
function schedule(){
 ...
 for(const unit of ['A','B']) for(const level of ['ground','upper']){
   ROOMS[unit][level].forEach(r=>rows.push({unit,level,name:r.name,kind:r.kind,dims:`${r.w}x${r.d}`,sf:sf(r),optional:!!r.optional,authorized:r.authorized!==false}));
 }
 return rows;
}
...
function analyze(){
 ...
 const checks={
  ...
  planningArea:{ok:cov.living.A>1700&&cov.living.B>1700,detail:`Authorized diagrammed room-zone area A ${cov.living.A.toFixed(0)} SF / B ${cov.living.B.toFixed(0)} SF across both floors; excludes garage, review options, and future wall-thickness deductions.`},
  optionalProgram:{ok:false,detail:`Potential over-garage option could add ${optional.A.toFixed(0)} SF to Unit A only after explicit design authority and credible upper-floor circulation. It is shown as a REVIEW option and is not counted in the passing area.`}
 };
 const verdict=Object.values(checks).every(x=>x.ok)?'PASS':'REVIEW';
 return {verdict,rev:REV,checks,schedule:schedule(),planningArea:cov.living,optionalArea:optional,freezeHash:D.LOCK.freezeHash};
}
...
function floor(level){
 ...
 ${rooms.map(r=>rect(r.x,r.y,r.w,r.d,roomFill(r.kind),r.optional?1.8:1.4,r.optional?'stroke-dasharray=\"6 4\" opacity=\".74\"':'')+text(r.x+r.w/2,r.y+r.d/2-0.5,r.name,r.optional?8:9)+text(r.x+r.w/2,r.y+r.d/2+1.5,r.optional?'REVIEW OPTION':`${r.w}′x${r.d}′`,8)).join('')}
