...
const ROOMS={
  ...
  upper:{
    A:[
      {id:'A-U-PRIMARY',name:'Primary Bedroom',x:94.5,y:5,w:16,d:13,kind:'bed'},
      {id:'A-U-BED2',name:'Bedroom 2',x:110.5,y:5,w:17.5,d:11,kind:'bed'},
      {id:'A-U-BED3',name:'Bedroom 3',x:110.5,y:16,w:17.5,d:9.5,kind:'bed'},
      {id:'A-U-BATH',name:'Hall Bath',x:94.5,y:18,w:8,d:8,kind:'bath',wet:true},
      {id:'A-U-PRIMARYBATH',name:'Primary Bath / WIC',x:102.5,y:18,w:8,d:13.25,kind:'bath',wet:true},
      {id:'A-U-STAIR',name:'Stair / Hall',x:94.5,y:26,w:8,d:5.25,kind:'hall',stair:true},
      {id:'A-U-HALL',name:'Upper Hall',x:110.5,y:25.5,w:17.5,d:5.75,kind:'hall'}
    ],
    B:[
...
function analyze(){
  ...
  for(const unit of ['A','B']){
    ...
    for(const level of ['ground','upper']){
      ...
      const roomArea=rooms.reduce((s,r)=>s+area(r),0),expected=shellArea(shell.poly);const areaDelta=roomArea-expected;
      if(outside.length)failures.push(`${unit} ${level} rooms outside shell: ${outside.join(', ')}`);if(overlaps.length)failures.push(`${unit} ${level} room overlaps`);if(Math.abs(areaDelta)>.01)failures.push(`${unit} ${level} room coverage delta ${areaDelta.toFixed(2)} sf`);
      levels[level]={ok:!outside.length&&!overlaps.length&&Math.abs(areaDelta)<=.01,roomAreaSf:+roomArea.toFixed(2),shellAreaSf:+expected.toFixed(2),areaDeltaSf:+areaDelta.toFixed(2),outside,overlaps};
    }
  ...
  return {rev:REV,verdict:failures.length?'FAIL':'PASS',failures,units,checks:{geometryOk:failures.length===0,bedrooms:units.A.bedrooms===3&&units.B.bedrooms===3,entries:units.A.entry.ok&&units.B.entry.ok,stairs:units.A.stairOverlapSf>=10&&units.B.stairOverlapSf>=10,wetCore:{ok:units.A.wetCore.ok&&units.B.wetCore.ok,blocking:false,status:(units.A.wetCore.ok&&units.B.wetCore.ok)?'OVERLAP_AVAILABLE':'ADVISORY',A:units.A.wetCore,B:units.B.wetCore}},note:'Planning-zone closure only. Areas are diagrammed gross shell/zone areas, not certified conditioned square footage or code approval.'}
}
...
function renderLevel(level){
  const W=1200,H=720,s=9.5,ox=-428,oy=72.5,a=analyze();let out=`<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Design 4 ${level} floor planning zones" data-plan-verdict="${a.verdict}" data-level="${level}"><rect width="${W}" height="${H}" fill="${COLORS.paper}"/><text x="62" y="50" font-family="Georgia,serif" font-size="29" font-weight="700" fill="${COLORS.outline}">${level==='ground'?'A-101 · Ground-floor planning zones':'A-102 · Upper-floor planning zones'}</text><text x="62" y="79" font-size="13" fill="${COLORS.muted}">Design 4 north-finger seed · room-program geometry ${a.verdict} · diagrammed zones, not certified conditioned area</text>`;
  for(const unit of ['B','A']){
    const shell=SHELLS[unit];out+=`<polygon points="${poly(shell.poly,s,ox,oy)}" fill="none" stroke="${COLORS.outline}" stroke-width="3"/>`;
    for(const r of ROOMS[level][unit]){out+=`<g data-room="${r.id}" data-unit="${unit}" data-kind="${r.kind}"><rect x="${(r.x*s+ox).toFixed(1)}" y="${(r.y*s+oy).toFixed(1)}" width="${(r.w*s).toFixed(1)}" height="${(r.d*s).toFixed(1)}" fill="${roomColor(r.kind)}" fill-opacity=".72" stroke="#405064" stroke-width="1.2"/><text x="${((r.x+r.w/2)*s+ox).toFixed(1)}" y="${((r.y+r.d/2)*s+oy).toFixed(1)}" text-anchor="middle" font-size="10" font-weight="800" fill="${COLORS.outline}">${r.name}</text><text x="${((r.x+r.w/2)*s+ox).toFixed(1)}" y="${((r.y+r.d/2)*s+oy+14).toFixed(1)}" text-anchor="middle" font-size="9" fill="${COLORS.muted}">${Math.round(area(r))} SF</text></g>`}
  }
  ...
  out+=`<g transform="translate(820,120)"><rect width="340" height="190" rx="12" fill="#fff" stroke="#d4d0c6"/><text x="18" y="30" font-size="13" font-weight="900" fill="${COLORS.outline}">PLAN CLOSURE · ${a.verdict}</text><text x="18" y="58" font-size="11" fill="${COLORS.outline}">Unit B: ${a.units.B.diagrammedGrossSf.toFixed(0)} SF gross diagram shell</text><text x="18" y="82" font-size="11" fill="${COLORS.outline}">Unit A: ${a.units.A.diagrammedGrossSf.toFixed(0)} SF gross diagram shell</text><text x="18" y="106" font-size="11" fill="${COLORS.outline}">3 bedrooms each · exterior entries verified</text><text x="18" y="130" font-size="11" fill="${COLORS.outline}">stairs overlap ground ↔ upper</text><text x="18" y="154" font-size="11" fill="${a.checks.wetCore.ok?COLORS.green:COLORS.red}">plumbing relationship: ${a.checks.wetCore.status}</text><text x="18" y="177" font-size="9.5" fill="${COLORS.muted}">Structural / code / MEP validation pending</text></g></svg>`;return out;
}
function renderBubble(){const a=analyze();return `<svg viewBox="0 0 1200 560" role="img" aria-label="Design 4 ownership and planning relationship diagram" data-bubble-verdict="${a.verdict}"><rect width="1200" height="560" fill="${COLORS.paper}"/><text x="62" y="50" font-family="Georgia,serif" font-size="29" font-weight="700" fill="${COLORS.outline}">A-103 · Ownership + room relationship</text><text x="62" y="79" font-size="13" fill="${COLORS.muted}">Detached garages remain separate accessory structures; each has a clear ownership relationship to its corresponding home.</text><g transform="translate(90,130)"><rect x="0" y="20" width="230" height="110" rx="22" fill="${COLORS.bed}" stroke="${COLORS.outline}"/><text x="115" y="65" text-anchor="middle" font-size="20" font-weight="900">HOME B</text><text x="115" y="92" text-anchor="middle" font-size="12">3 BR · ~1,784 SF shell</text><rect x="0" y="240" width="230" height="90" rx="20" fill="#c7d9c4" stroke="${COLORS.outline}"/><text x="115" y="282" text-anchor="middle" font-size="18" font-weight="900">GARAGE B</text><text x="115" y="307" text-anchor="middle" font-size="12">22×22 · detached</text><path d="M115 130 L115 240" stroke="${COLORS.red}" stroke-width="4" stroke-dasharray="9 6"/><text x="132" y="192" font-size="11" font-weight="800" fill="${COLORS.red}">ownership / walk</text></g><g transform="translate(560,130)"><rect x="0" y="20" width="230" height="110" rx="22" fill="${COLORS.kitchen}" stroke="${COLORS.outline}"/><text x="115" y="65" text-anchor="middle" font-size="20" font-weight="900">HOME A</text><text x="115" y="92" text-anchor="middle" font-size="12">3 BR · ~1,759 SF shell</text><rect x="0" y="240" width="230" height="90" rx="20" fill="#c7d9c4" stroke="${COLORS.outline}"/><text x="115" y="282" text-anchor="middle" font-size="18" font-weight="900">GARAGE A</text><text x="115" y="307" text-anchor="middle" font-size="12">22×22 · detached</text><path d="M115 130 L115 240" stroke="${COLORS.red}" stroke-width="4" stroke-dasharray="9 6"/><text x="132" y="192" font-size="11" font-weight="800" fill="${COLORS.red}">ownership / walk</text></g><g transform="translate(870,155)"><rect width="270" height="275" rx="18" fill="#fff" stroke="#d4d0c6"/><text x="18" y="32" font-size="13" font-weight="900">CURRENT READ</text><text x="18" y="62" font-size="11">✓ two homes</text><text x="18" y="88" font-size="11">✓ distinct exterior entries</text><text x="18" y="114" font-size="11">✓ 3 bedrooms each</text><text x="18" y="140" font-size="11">✓ north garage → Home B</text><text x="18" y="166" font-size="11">✓ south garage → Home A</text><text x="18" y="192" font-size="11">✓ detached garage relationship explicit</text><text x="18" y="222" font-size="11" fill="${a.checks.wetCore.ok?COLORS.green:COLORS.red}">△ plumbing: ${a.checks.wetCore.status}</text><text x="18" y="248" font-size="9.5" fill="${COLORS.muted}">design-development planning only</text></g></svg>`}

const api={REV,TARGET,SHELLS,ENTRIES,ROOMS,analyze,renderLevel,renderBubble,area,shellArea};
