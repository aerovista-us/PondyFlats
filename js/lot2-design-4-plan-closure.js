...
function renderLevel(level){
  ...
  out+=`<g transform="translate(800,120)"><rect width="340" height="190" rx="12" fill="#fff" stroke="#d4d0c6"/><text x="18" y="30" font-size="13" font-weight="900" fill="${COLORS.outline}">PLAN CLOSURE · ${a.verdict}</text><text x="18" y="58" font-size="11" fill="${COLORS.outline}">Unit B: ${a.units.B.diagrammedGrossSf.toFixed(0)} SF gross diagram shell</text><text x="18" y="82" font-size="11" fill="${COLORS.outline}">Unit A: ${a.units.A.diagrammedGrossSf.toFixed(0)} SF gross diagram shell</text><text x="18" y="106" font-size="11" fill="${COLORS.outline}">3 bedrooms each · exterior entries verified</text><text x="18" y="130" font-size="11" fill="${COLORS.outline}">stairs overlap ground ↔ upper</text><text x="18" y="154" font-size="11" fill="${a.checks.wetCore.ok?COLORS.green:COLORS.red}">plumbing relationship: ${a.checks.wetCore.status}</text><text x="18" y="177" font-size="9.5" fill="${COLORS.muted}">Structural / code / MEP validation pending</text></g></svg>`;return out;
}
